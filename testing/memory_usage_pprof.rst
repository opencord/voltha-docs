...
This page shows how to analize the memory consumption of a VOLTHA container (written in Go) by using a
custom built image of the container and the ``pprof`` tool, provided by the GO Language.

Memory Garbage collection in GO
-------------------------------

The release of unneeded heap memory in Go is handled by the so-called Garbage Collector (GC),
thus there is no way to explicitly release heap memory areas in code.
The GC only recognizes a heap memory area as being released when no more references to this
memory area exist. If object hold mutual references to each the GC does not free their memory. To solve this issue,
the respective references of the individual objects need to be placed to nil.
An example was an older implementation of voltha-openonu-adapter-go, that did not de-reference object after deletion
of the corresponding ONU device. To solve this issue, the respective references of the individual objects to each other
are be eliminated upon ONU device deletion.
In the following the procedure, analysis and correction are briefly described.
...
.. code:: bash
  ...
  NAME   NAME                                   TYPE      CLUSTER EXTERNAL PORT(S)  AGE   SPACE                                                   -IP     -IP
  …
...
Following is an example of hot to get the heap usage of the container and save it to a file:
...
by using the ``top`` command one can see which are the main causes of memory leaks,
...
than 850 instances of ``onuDeviceEntry`` not deleted by the GC. Furthermore, we know that numerous objects are storing
a pointer to the corresponding instance of ``onuDeviceEntry``. Among these is the parent ``deviceHandler``
to which the ``onuDeviceEntry`` holds a reference too (``onuDeviceEntry.baseDeviceHandler``).
We conclude that there is a network of mutual references.

To resolve this, the mutual references of the different object instances of a particular ONU have to be removed
after an ONU has been deleted from management by the adapter
(see `Gerrit 30640 patch <https://gerrit.opencord.org/c/voltha-openonu-adapter-go/+/30640>`_).
After applying the patch the same test with hundreds of cycles of startup and deletion of multiple BBSIM ONUs shows
that the issue has been solved. Having a look at the retrieved pprof data shows a significant
