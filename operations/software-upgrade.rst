...
It is also expected that new versions of the different components are available to the operator that performs the upgrade.
...
//TODO can we just re-deploy kafka and point the different components to it ? will it work with minor service interruption ?
...
The `openolt agent` on the box can be upgrade without having to teardown all the VOLTHA stack to which the OLT was connected.
Again here we make the ditinction of a minor update and a major update of the openolt agent.
...
//TODO do we need to push the TP here ?

After these steps are done VOLTHA will re-receive the OLT connection and re-provision data accordingly.

Major update
------------
A major update will require the OLT to be deleted from VOLTHA to ensure no un-consistent data is stored.
