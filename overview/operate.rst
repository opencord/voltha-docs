...
In this page we assume that you a VOLTHA POD (either Physical or Virtual)
up and running.
...
If you deployed a Virtual cluster you can create a BBSim with:
...
Regardless of the OLT the command to ``enable`` it is always the same:
...
If the use-case you installed expect EAPOL based authentication you want to make
...
*Note that authentication is not a requirement of subscriber provisioning*

The process that is referred to as ``Subscriber provisioning`` is the process
that causes the flows to be created in ONOS and the data plane path to be configured
in the device.

In order to provision a subscriber you need to identify it. In ONOS a subscriber
is viewed as an enabled port on the logical switch that VOLTHA exposes, for example:
...
that an IP has correctly been allocated.
...
If you deployed a **Virtual POD** with a BBSim OLT you are. BBSim does not support
data plane emulation at the moment.

If you deployed a **Physical POD** then you are now able to reach the internet from
your client.
